function stimERPs_withBootstrap_v2(PatientID)
    %TODO: Set metric in line 11 and load corresponding file out of lines
    %22-24
    ...
    if computeMaps
        ...
        % load(fullfile(patientdatapath,'Processed_BioPacData',strcat(PatientID,'_cvxEDAOutoutByBlock.mat')),'cvxOutputByBlock'); %.r variable for raw phasic, not z scored. Does not include between-stim data
        ...
        % load(fullfile(patientdatapath,'Processed_BioPacData',strcat(PatientID,'_ECG_IBI_OutoutByBlock.mat')),'ibiOutputByBlock'); %For uncleaned, original RSA: .RSA_melSpec_12s variable
        ...
        % load(fullfile(patientdatapath,'Processed_BioPacData',strcat(PatientID,'_Suff_RSA_Cleaned_FullTimeCourse_zScored.mat')),'rsa_cleaned_melSpec_16s_zData'); %For cleaned RSA data
        ...
        unique_blocks = unique(blocks,'sorted');
            ...
            for j = 1:length(thisCurrent_sources) %Sort by source electrode
                ...
                for k = 1:length(thisCurrentSource_targets) %Sort by target electrode
                    ...
                    for l = 1:length(thisCurrentSourceTarget_frequencies) %Sort by frequency
                        ...
                        [flag source_elec target_elec] = checkAdjacency(source_elec,target_elec); %Checks if source and target electrodes are directly adjacent in ascending order
                        ...
                        for m = 1:length(thisCurrentSourceTargetFreq_inds)
                            switch metric
                                ...
                                case 'RSA'
                                    ...
                                    % timecourseLengths = [timecourseLengths length(out_movie(thisCurrentSourceTargetFreq_inds(m)).processed_biopacdata.RSA.RSA_melSpec_16s.rawData)];
                                case 'IBI'
                        ...
                        switch metric
                            case 'phasic'
                                ...
                                [allMapInfo(i).bins(numBins).controlTimecourse allMapInfo(i).bins(numBins).controlSemTimecourse] = stimBootstrap_v2(full_eda,length(thisCurrentSourceTargetFreq_inds),fs,minTimecourseLength,200);
                            case 'RSA'
                                ...
                                [allMapInfo(i).bins(numBins).controlTimecourse allMapInfo(i).bins(numBins).controlSemTimecourse] = stimBootstrap_v2(full_rsa,length(thisCurrentSourceTargetFreq_inds),fs,minTimecourseLength,200);
                        end  
                        numBins = numBins + 1;
                    end
                end
    ...
    for i = 1:length(allMapInfo)
        ...
        for j = 1:length(allMapInfo(i).bins)
            ...
            p_real = stdshade_MeanSem(allMapInfo(i).bins(j).meanTimecourse,allMapInfo(i).bins(j).semTimecourse,.3,colors(i,:));
            hold on;
        ...
        saveas(figi, strrep(strjoin([{patientdatapath} '/AnalyzedData/SufficiencyStimMaps/' PatientID '_SuffStim_' metric '_' figname]), ' ', ''), 'png');
        ...
        if ~isempty(flagged)
            ...
            saveas(flagi, strrep(strjoin([{patientdatapath} '/AnalyzedData/SufficiencyStimMaps/' PatientID '_SuffStim_' metric '_' figname]), ' ', ''), 'png');
...
function [flag,source_elec,target_elec] = checkAdjacency(source,target)
    ...
    if ~strcmp(source(1:source_num-1),target(1:target_num-1)) %Check same site first
        flag = 1;
    elseif str2num(source(source_num:end)) == str2num(target(target_num:end)) + 1 %E.g. 10->9
        flag = 0;
        source_elec = target;
        target_elec = source;
